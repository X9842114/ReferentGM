...
class MetallicShaderMount {
  ...
  constructor(
    ...
    else this.renderFrame(performance.now());
  }
  ...
  private setupObservers() {
    ...
    if (typeof IntersectionObserver !== "undefined") {
      this.intersectionObserver = new IntersectionObserver(
        ...
        { root: null, rootMargin: "160px 80px", threshold: 0 },
      );
  ...
  private handleResize = () => {
    ...
    const width = Math.max(this.parent.clientWidth, Math.round(rect.width));
    const height = Math.max(this.parent.clientHeight, Math.round(rect.height));
  ...
  private pushUniforms() {
    ...
    gl.uniform1f(this.locations.u_fit ?? null, 2);
    gl.uniform1f(this.locations.u_rotation ?? null, 0);
    gl.uniform1f(this.locations.u_offsetX ?? null, 0);
    gl.uniform1f(this.locations.u_offsetY ?? null, 0);
    gl.uniform1f(this.locations.u_originX ?? null, 0.5);
    gl.uniform1f(this.locations.u_originY ?? null, 0.5);
    gl.uniform1f(this.locations.u_worldWidth ?? null, this.cssWidth);
    gl.uniform1f(this.locations.u_worldHeight ?? null, this.cssHeight);
  }
  ...
  private renderFrame = (currentTime: number) => {
    ...
    if (this.speed !== 0 && this.isInViewport) this.requestRender();
  };
...
const SETTLE = "transition-[transform,box-shadow] duration-200 ease-out";
...
export function MetallicButton({
  ...
  return (
    <div
      className={`${fullWidth ? "relative block w-full" : "relative inline-flex w-fit max-w-full shrink-0"} ${className}`}
    >
      <button
        ref={buttonRef}
        type={type}
        ...
        className={`relative isolate inline-flex h-[2.875rem] items-center justify-center rounded-full border-0 bg-transparent p-0 outline-none ${
          isIcon ? "w-[2.875rem]" : fullWidth ? "w-full" : "w-auto min-w-[8.875rem]"
        } ${pressShift} ${SETTLE} ${rimClass} ${
          disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"
        }`}
      >
        <span
          ref={surfaceRef}
          className="metallic-button-canvas pointer-events-none absolute inset-0 overflow-hidden rounded-full"
        />
        <span
          className={`pointer-events-none absolute inset-[3px] z-[1] rounded-full bg-[linear-gradient(180deg,#262626_0%,#070707_100%)] ${
            isPressed ? FACE_PRESSED : ""
          }`}
        />
        <span
          className={`relative z-[2] inline-flex items-center justify-center px-5 text-sm font-medium whitespace-nowrap text-white [text-shadow:0px_1px_2px_rgba(0,0,0,0.55)] ${
            isIcon ? "px-0" : ""
          }`}
        >
          {isIcon ? (
            <Sparkles className="size-4 drop-shadow-[0px_1px_2px_rgba(0,0,0,0.5)]" />
          ) : (
            label
          )}
        </span>
        {ripples.map((ripple) => (
          <span
            key={ripple.id}
            className="pointer-events-none absolute z-20 size-5 rounded-full bg-[radial-gradient(circle,rgba(255,255,255,0.4)_0%,rgba(255,255,255,0)_70%)] animate-[metallic-button-ripple_0.6s_ease-out]"
            style={{
              left: `${ripple.x}px`,
              top: `${ripple.y}px`,
            }}
